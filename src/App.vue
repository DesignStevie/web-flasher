...
<template>
  <VApp>
    <VLayout>
      ...
      >
        <template>
          <v-img src=".././public/background_shapes.png" />
        </template>

        <div class="logoContainer" style="padding: 20px">
          <svg
            class="logo"
            fill="#fff"
            viewBox="0 0 512 512"
      ...
      <VMain>
        <VFadeTransition mode="out-in">
          <VContainer max-width="880px" v-if="!store.targetType">
            <FirmwareSelect />
          </VContainer>
          <VContainer max-width="880px" v-else>
            <VStepper
              v-model="store.currentStep"
              :items="['Hardware', 'Options', 'Flashing']"
              hideActions
            >
              <template v-slot:item.1>
                <MainHardwareSelect v-if="store.firmware === 'firmware'" />
                <VRXHardwareSelect
                  vendor-label="Transmitter Module"
                  v-if="store.targetType === 'txbp'"
                />
                <VRXHardwareSelect
                  vendor-label="VRx Type"
                  v-if="store.targetType === 'vrx'"
                />
                <VRXHardwareSelect
                  vendor-label="Antenna Tracker Type"
                  v-if="store.targetType === 'aat'"
                />
                <VRXHardwareSelect
                  vendor-label="Timer Type"
                  v-if="store.targetType === 'timer'"
                />
              </template>
              ...
              <template v-slot:item.3>
                ...
                <STLinkFlash
                  v-else-if="store.options.flashMethod === 'stlink'"
                />
                <SerialFlash v-else />
              </template>
              <VStepperActions
